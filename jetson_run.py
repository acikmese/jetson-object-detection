...
def run(weights=YOLO_ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    images_dir = save_dir.joinpath('images')
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if save_img and (utc_time - utc_prev_time).total_seconds() >= save_interval:
                tf = utc_time
                f_name = f"{p.stem}_{tf.year}_{tf.month}_{tf.day}_{tf.hour}_{tf.minute}_{tf.second}"
                img_path_name = str(images_dir) + '/' + f_name + '.jpg'
                cv2.imwrite(img_path_name, im0, [cv2.IMWRITE_JPEG_QUALITY, 90])
