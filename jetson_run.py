...
from extra_utils import calculate_distance, zip_with_datetime, set_log_file_handler, get_version
...
def run(weights=YOLO_ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    if not nosave:
        ...
        # Set logs
        log_path = Path(log_dir) / 'logs.log'  # path to log file
                    ...
                    if not nosave:
                        if not os.path.isfile(prod_text_path) or os.stat(prod_text_path).st_size == 0:
                            with open(prod_text_path, 'a') as f:
                                out_txt = f"datetime_utc,frame_id,id_type,id,confidence,x_center,y_center," \
                                          f"x_width,y_width,x_norm_center,y_norm_center,x_norm_width,y_norm_width," \
